 ...
 * Alias:   CRC-8/ATM
 *****************************************************************************/
static uint8_t crc8ITU_table[256] = {
0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B, 0x12, 0x15, 0x38, 0x3F, 0x36, 0x31, 0x24, 0x23, 0x2A, 0x2D,
0x70, 0x77, 0x7E, 0x79, 0x6C, 0x6B, 0x62, 0x65, 0x48, 0x4F, 0x46, 0x41, 0x54, 0x53, 0x5A, 0x5D,
0xE0, 0xE7, 0xEE, 0xE9, 0xFC, 0xFB, 0xF2, 0xF5, 0xD8, 0xDF, 0xD6, 0xD1, 0xC4, 0xC3, 0xCA, 0xCD,
0x90, 0x97, 0x9E, 0x99, 0x8C, 0x8B, 0x82, 0x85, 0xA8, 0xAF, 0xA6, 0xA1, 0xB4, 0xB3, 0xBA, 0xBD,
...
void crc8_itu_GenerateTable()
{
    ...
    /* 用于打印数组 */
//    printf("uint8_t crc8ITU_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%02X, ",crc8ITU_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
/* 用于打印数组 */
//    printf("uint8_t crc8ROHC_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
...
void crc8_maxim_GenerateTable()
{
    ...
    /* 用于打印数组 */
//    printf("uint8_t crc8MAXIM_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
...
void crc16_ibm_GenerateTable()
{
    ...
    /* 用于打印数组 */
//    printf("uint8_t crc16IBM_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
 ...
 * Note:
 *****************************************************************************/
static uint16_t crc16MAXIM_table[256] = {
0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241, 0xC601, 0x06C0, 0x0780, 0xC741, 0x0500, 0xC5C1, 0xC481, 0x0440,
0xCC01, 0x0CC0, 0x0D80, 0xCD41, 0x0F00, 0xCFC1, 0xCE81, 0x0E40, 0x0A00, 0xCAC1, 0xCB81, 0x0B40, 0xC901, 0x09C0, 0x0880, 0xC841,
0xD801, 0x18C0, 0x1980, 0xD941, 0x1B00, 0xDBC1, 0xDA81, 0x1A40, 0x1E00, 0xDEC1, 0xDF81, 0x1F40, 0xDD01, 0x1DC0, 0x1C80, 0xDC41,
0x1400, 0xD4C1, 0xD581, 0x1540, 0xD701, 0x17C0, 0x1680, 0xD641, 0xD201, 0x12C0, 0x1380, 0xD341, 0x1100, 0xD1C1, 0xD081, 0x1040,
...
void crc16_maxim_GenerateTable()
{
    uint16_t crc = 0;
    for(uint16_t i=0; i<256; i++){
        crc = i;               // Initial value
        ...
        crc16MAXIM_table[i] = crc;
    }
//    /* 用于打印数组 */
//    printf("uint16_t crc16MAXIM_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%04X, ",crc16MAXIM_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
uint16_t crc16_maxim_calc(uint8_t *data, uint32_t length)
{
    ...
    {
        tmp =  (0x00FF & crc) ^ (*data++);    // crc ^= *data; data++;
        crc = (crc >> 8) ^ crc16MAXIM_table[tmp&0xFF];
    }
 ...
 * Note:
 *****************************************************************************/
static uint16_t crc16USB_table[256] = {
0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241, 0xC601, 0x06C0, 0x0780, 0xC741, 0x0500, 0xC5C1, 0xC481, 0x0440,
0xCC01, 0x0CC0, 0x0D80, 0xCD41, 0x0F00, 0xCFC1, 0xCE81, 0x0E40, 0x0A00, 0xCAC1, 0xCB81, 0x0B40, 0xC901, 0x09C0, 0x0880, 0xC841,
0xD801, 0x18C0, 0x1980, 0xD941, 0x1B00, 0xDBC1, 0xDA81, 0x1A40, 0x1E00, 0xDEC1, 0xDF81, 0x1F40, 0xDD01, 0x1DC0, 0x1C80, 0xDC41,
0x1400, 0xD4C1, 0xD581, 0x1540, 0xD701, 0x17C0, 0x1680, 0xD641, 0xD201, 0x12C0, 0x1380, 0xD341, 0x1100, 0xD1C1, 0xD081, 0x1040,
...
void crc16_usb_GenerateTable()
{
    ...
    for(uint16_t i=0; i<256; i++){
        ...
        crc16USB_table[i] = crc;
    }
    /* 用于打印数组 */
//    printf("static uint16_t crc16USB_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%04X, ",crc16USB_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
uint16_t crc16_usb_calc(uint8_t *data, uint32_t length)
{
    ...
    {
        tmp =  (0x00FF & crc) ^ (*data++);    // crc ^= *data; data++;
        crc = (crc >> 8) ^ crc16USB_table[tmp&0xFF];
    }
    return ~crc; // crc ^ 0xFFFF
}
...
void crc16_modbus_GenerateTable()
{
    uint16_t crc = 0;
    for(uint16_t i=0; i<256; i++){
        crc = i;               // Initial value
        ...
        crc16MODBUS_table[i] = crc;
    }
    /* 用于打印数组 */
//    printf("static uint16_t crc16MODBUS_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%04X, ",crc16MODBUS_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
static uint16_t crc16X25_table[256] = {
0x0000, 0x1189, 0x2312, 0x329B, 0x4624, 0x57AD, 0x6536, 0x74BF, 0x8C48, 0x9DC1, 0xAF5A, 0xBED3, 0xCA6C, 0xDBE5, 0xE97E, 0xF8F7,
0x1081, 0x0108, 0x3393, 0x221A, 0x56A5, 0x472C, 0x75B7, 0x643E, 0x9CC9, 0x8D40, 0xBFDB, 0xAE52, 0xDAED, 0xCB64, 0xF9FF, 0xE876,
0x2102, 0x308B, 0x0210, 0x1399, 0x6726, 0x76AF, 0x4434, 0x55BD, 0xAD4A, 0xBCC3, 0x8E58, 0x9FD1, 0xEB6E, 0xFAE7, 0xC87C, 0xD9F5,
0x3183, 0x200A, 0x1291, 0x0318, 0x77A7, 0x662E, 0x54B5, 0x453C, 0xBDCB, 0xAC42, 0x9ED9, 0x8F50, 0xFBEF, 0xEA66, 0xD8FD, 0xC974,
...
void crc16_x25_GenerateTable()
{
    ...
    for(uint16_t i=0; i<256; i++){
        ...
        crc16X25_table[i] = crc;
    }
    /* 用于打印数组 */
//    printf("static uint16_t crc16X25_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%04X, ",crc16X25_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
uint16_t crc16_x25_calc(uint8_t *data, uint32_t length)
{
    ...
    {
        tmp =  (0x00FF & crc) ^ (*data++);    // crc ^= *data; data++;
        crc = (crc >> 8) ^ crc16X25_table[tmp&0xFF];
    }
    return ~crc; // crc ^ 0xFFFF
}
 ...
 *****************************************************************************/
static uint16_t crc16XMODEM_table[256] = {
0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7, 0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6, 0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
0x2462, 0x3443, 0x0420, 0x1401, 0x64E6, 0x74C7, 0x44A4, 0x5485, 0xA56A, 0xB54B, 0x8528, 0x9509, 0xE5EE, 0xF5CF, 0xC5AC, 0xD58D,
0x3653, 0x2672, 0x1611, 0x0630, 0x76D7, 0x66F6, 0x5695, 0x46B4, 0xB75B, 0xA77A, 0x9719, 0x8738, 0xF7DF, 0xE7FE, 0xD79D, 0xC7BC,
...
void crc16_xmodem_GenerateTable()
{
    uint16_t crc = 0;
    for(uint16_t i=0; i<256; i++){
        crc = i<<8;               // Initial value
        ...
        crc16XMODEM_table[i] = crc;
    }
    /* 用于打印数组 */
//    printf("static uint16_t crc16XMODEM_table[256] = {\n");
//    for(uint16_t i=0; i<256; i++){
//        printf("0x%04X, ",crc16XMODEM_table[i]);
//        if( (i+1)%16 == 0 ){
//            printf("\n");
//        }
//    }
...
uint16_t crc16_xmodem_calc(uint8_t *data, uint32_t length)
{
    ...
    {
        tmp = (crc >> 8) ^ (*data++);    // crc ^= *data; data++;
        crc = (crc << 8) ^ crc16XMODEM_table[tmp&0xFF];
    }
    return crc; // crc
}
